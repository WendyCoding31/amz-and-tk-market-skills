...
STRATEGIES = {
    "S1": {"ad_type": "auto", "default_bid": 0.05, "auto_targets": ["close-match", "loose-match"]},
    "S2": {"ad_type": "auto", "default_bid": 0.25, "auto_targets": ["close-match", "substitutes"]},
    "S3": {"ad_type": "auto", "default_bid": 0.2, "auto_targets": AUTO_TARGETS},
    "S4": {"ad_type": "sp_keyword", "default_bid": 0.35, "match_types": ["exact"]},
    "S5": {"ad_type": "sp_keyword", "default_bid": 0.02, "match_types": ["broad"]},
    "S6": {"ad_type": "sp_keyword", "default_bid": 0.02, "match_types": ["broad"], "default_grouping": "rank"},
    "S7": {"ad_type": "sp_keyword", "default_bid": 0.15, "match_types": ["broad"], "default_grouping": "rank"},
    "S8": {"ad_type": "sp_keyword", "default_bid": 0.2, "match_types": ["broad"]},
    "S9": {"ad_type": "sp_keyword", "default_bid": 0.15, "match_types": ["broad"]},
    "S10": {"ad_type": "sp_keyword", "default_bid": 0.2, "match_types": ["broad", "phrase", "exact"]},
    "S11": {"ad_type": "sp_keyword", "default_bid": 0.3, "match_types": ["phrase"]},
    "S12": {"ad_type": "sp_keyword", "default_bid": 0.3, "match_types": ["phrase"]},
    "S13": {"ad_type": "sp_keyword", "default_bid": 0.2, "match_types": ["phrase"]},
    "S14": {"ad_type": "sp_keyword", "default_bid": 0.2, "match_types": ["phrase"]},
    "S15": {"ad_type": "sp_keyword", "default_bid": 0.2, "match_types": ["broad"], "default_grouping": "rank"},
    "S16": {"ad_type": "sp_keyword", "default_bid": 0.2, "match_types": ["phrase"]},
    "S17": {"ad_type": "sp_keyword", "default_bid": 0.25, "match_types": ["exact"], "default_grouping": "rank"},
    "S18": {"ad_type": "sp_keyword", "default_bid": 0.5, "match_types": ["phrase"]},
    "S19": {"ad_type": "sp_keyword", "default_bid": 0.05, "match_types": ["exact", "phrase", "broad"]},
    "S20": {"ad_type": "asin", "default_bid": 0.05},
    "S21": {"ad_type": "asin", "default_bid": 0.1},
    "S22": {"ad_type": "sp_keyword", "default_bid": 0.2, "match_types": ["broad"], "default_grouping": "rank"},
    "S23": {"ad_type": "sp_keyword", "default_bid": 0.5, "match_types": ["broad"]},
    "S24": {"ad_type": "sp_keyword", "default_bid": 0.35, "match_types": ["exact", "phrase"]},
    "S25": {"ad_type": "sp_keyword", "default_bid": 0.2, "match_types": ["phrase"], "default_grouping": "rank"},
    "S26": {"ad_type": "sp_keyword", "default_bid": 0.35, "match_types": ["exact"]},
    "S27": {"ad_type": "asin", "default_bid": 0.1},
}

STRATEGY_KEYWORD_LIMITS = {
    "S22": 49,
}
...
def normalize_strategy(value):
    ...
    if re.fullmatch(r"\d+", strategy):
        strategy = f"S{strategy}"
    if strategy not in STRATEGIES:
        raise InputError(f"Unknown strategy: {value}")
    return strategy
...
def normalize_ad_type(value, strategy):
    if not value:
        return STRATEGIES[strategy]["ad_type"]
    aliases = {
        "sp": "sp_keyword",
        "keyword": "sp_keyword",
        "keywords": "sp_keyword",
...
def normalize_match_types(value, strategy):
    if value is None:
        value = STRATEGIES[strategy].get("match_types", ["broad"])
...
def normalize_auto_targets(spec, strategy, default_bid):
    targets = spec.get("auto_targets")
    if not targets:
        return [{"expression": t, "bid": default_bid} for t in STRATEGIES[strategy].get("auto_targets", AUTO_TARGETS)]
...
def campaign_base(strategy, sku, start_date, kind, match_or_target, bucket=None, root=None, seq=1):
    parts = [strategy]
    if root:
        parts.append(root)
...
def process_keyword_campaign(ws, spec, strategy, sku, start_date, stats):
    keywords = normalize_keywords(spec.get("keywords"))
    keyword_limit = STRATEGY_KEYWORD_LIMITS.get(strategy)
    if keyword_limit is not None and len(keywords) > keyword_limit:
        raise InputError(
            f"{strategy} must use fewer than 50 keywords; got {len(keywords)}. "
            f"Filter the source to at most {keyword_limit} keywords."
        )
    match_types = normalize_match_types(spec.get("match_types") or spec.get("match_type"), strategy)
    grouping = spec.get("grouping", STRATEGIES[strategy].get("default_grouping", "none"))
    default_bid = to_number(
        spec.get("ad_group_default_bid", spec.get("default_bid", spec.get("bid"))),
        STRATEGIES[strategy]["default_bid"],
    )
    ...
    for match_type in match_types:
        ...
        for seq, (bucket, root, rows) in enumerate(groups, start=1):
            if not rows:
                continue
            campaign_id = campaign_base(strategy, sku, start_date, "SPKW", match_type, bucket=bucket, root=root, seq=seq)
...
def process_asin_campaign(ws, spec, strategy, sku, start_date, stats):
    ...
    for seq, start in enumerate(range(0, len(asins), per_campaign), start=1):
        ...
        campaign_id = campaign_base(strategy, sku, start_date, "SPASIN", "ASIN", seq=seq)
        ad_group_id = f"{campaign_id}_AG01"
        add_campaign_shell(
            ws,
            campaign_id,
...
def process_auto_campaign(ws, spec, strategy, sku, start_date, stats):
    default_bid = to_number(
        spec.get("ad_group_default_bid", spec.get("default_bid", spec.get("bid"))),
        STRATEGIES[strategy]["default_bid"],
    )
    ...
    tos_pct = spec.get("top_of_search_percentage")
    tos_pct = int(tos_pct) if tos_pct not in (None, "") else None
    targets = normalize_auto_targets(spec, strategy, default_bid)

    campaign_id = campaign_base(strategy, sku, start_date, "SPAUTO", "AUTO", seq=1)
    ad_group_id = f"{campaign_id}_AG01"
    add_campaign_shell(
        ws,
        campaign_id,
...
def build_workbook(data, template_path, output_path):
    ...
    for spec in campaigns:
        strategy = normalize_strategy(spec.get("strategy"))
        ad_type = normalize_ad_type(spec.get("ad_type"), strategy)
        expected = STRATEGIES[strategy]["ad_type"]
        if strategy == "S21" and ad_type != "asin":
            raise InputError("S21 must be converted to an ASIN list and generated as ad_type='asin'")
        if ad_type != expected and strategy != "S21":
            raise InputError(f"{strategy} expects ad_type={expected}, got {ad_type}")
        if ad_type == "sp_keyword":
            process_keyword_campaign(ws, spec, strategy, sku, start_date, stats)
        elif ad_type == "asin":
            process_asin_campaign(ws, spec, strategy, sku, start_date, stats)
        elif ad_type == "auto":
            process_auto_campaign(ws, spec, strategy, sku, start_date, stats)
        else:
