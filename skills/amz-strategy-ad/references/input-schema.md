...
For `amz-strategy-ad`, prefer ASCII `campaign_label` values that include the confirmed strategy code, such as `S1_AUTO_SHOULU`, `S24_RANK_EXACT`, or `S20_ASIN_ASSOC`.

For `拓关联` ASIN campaigns, use `asins_per_campaign: 10` unless the user confirms another grouping size.
